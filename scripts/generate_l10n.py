...
add("statCurrentStreak", "Current Streak", "Güncel Seri", "Aktuelle Serie", "Série actuelle", "Racha actual", "Serie attuale", "現在の連続", "当前连续", "현재 연속", "Huidige reeks", "Текущая серия")
add("statBestStreak", "Best Streak", "En İyi Seri", "Beste Serie", "Meilleure série", "Mejor racha", "Miglior serie", "最長連続", "最长连续", "최고 연속", "Beste reeks", "Лучшая серия")
add("statStreakDays", "{count} days", "{count} gün", "{count} Tage", "{count} jours", "{count} días", "{count} giorni", "{count} 日", "{count} 天", "{count}일", "{count} dagen", "{count} дн.")
...
add("friendCode", "Friend Code", "Arkadaş Kodu", "Freundcode", "Code ami", "Código de amigo", "Codice amico", "フレンドコード", "好友码", "친구 코드", "Vriendcode", "Код друга")
add("yourFriendCode", "Your friend code", "Arkadaş kodun", "Dein Freundcode", "Votre code ami", "Tu código de amigo", "Il tuo codice amico", "あなたのフレンドコード", "你的好友码", "내 친구 코드", "Jouw vriendcode", "Ваш код друга")
add("friendCodeCopied", "Friend code copied", "Arkadaş kodu kopyalandı", "Freundcode kopiert", "Code ami copié", "Código copiado", "Codice copiato", "フレンドコードをコピーしました", "好友码已复制", "친구 코드가 복사됨", "Vriendcode gekopieerd", "Код друга скопирован")
add("addFriendHint", "Enter your friend's 6-digit code", "Arkadaşının 6 haneli kodunu gir", "Gib den 6-stelligen Code deines Freundes ein", "Entrez le code à 6 chiffres de votre ami", "Introduce el código de 6 dígitos de tu amigo", "Inserisci il codice a 6 cifre del tuo amico", "友達の6桁コードを入力", "输入好友的6位数字码", "친구의 6자리 코드를 입력하세요", "Voer de 6-cijferige code van je vriend in", "Введите 6-значный код друга")
add("invalidFriendCode", "Enter a valid 6-digit code", "Geçerli 6 haneli bir kod girin", "Gib einen gültigen 6-stelligen Code ein", "Entrez un code à 6 chiffres valide", "Introduce un código de 6 dígitos válido", "Inserisci un codice a 6 cifre valido", "有効な6桁のコードを入力してください", "请输入有效的6位数字码", "유효한 6자리 코드를 입력하세요", "Voer een geldige 6-cijferige code in", "Введите корректный 6-значный код")
add("friendCodeNotFound", "No player found with this code", "Bu kodla oyuncu bulunamadı", "Kein Spieler mit diesem Code gefunden", "Aucun joueur trouvé avec ce code", "No se encontró ningún jugador con este código", "Nessun giocatore trovato con questo codice", "このコードのプレイヤーは見つかりません", "未找到使用此代码的玩家", "이 코드의 플레이어를 찾을 수 없습니다", "Geen speler gevonden met deze code", "Игрок с таким кодом не найден")
add("cannotAddYourself", "You can't add yourself", "Kendini ekleyemezsin", "Du kannst dich nicht selbst hinzufügen", "Vous ne pouvez pas vous ajouter", "No puedes añadirte a ti mismo", "Non puoi aggiungere te stesso", "自分自身は追加できません", "不能添加自己", "자기 자신은 추가할 수 없습니다", "Je kunt jezelf niet toevoegen", "Нельзя добавить себя")
add("alreadyFriends", "You are already friends", "Zaten arkadaşsınız", "Ihr seid bereits Freunde", "Vous êtes déjà amis", "Ya sois amigos", "Siete già amici", "すでに友達です", "你们已经是好友了", "이미 친구입니다", "Jullie zijn al vrienden", "Вы уже друзья")
add("friendAdded", "{name} added as friend", "{name} arkadaş olarak eklendi", "{name} als Freund hinzugefügt", "{name} ajouté(e) comme ami", "{name} añadido como amigo", "{name} aggiunto come amico", "{name} を友達に追加しました", "已将 {name} 加为好友", "{name} 님이 친구로 추가됨", "{name} toegevoegd als vriend", "{name} добавлен(а) в друзья")
add("friendRemoved", "Friend removed", "Arkadaş kaldırıldı", "Freund entfernt", "Ami retiré", "Amigo eliminado", "Amico rimosso", "友達を削除しました", "已移除好友", "친구가 삭제됨", "Vriend verwijderd", "Друг удалён")
add("removeFriend", "Remove Friend", "Arkadaşı Kaldır", "Freund entfernen", "Retirer l'ami", "Eliminar amigo", "Rimuovi amico", "友達を削除", "删除好友", "친구 삭제", "Vriend verwijderen", "Удалить друга")
add("removeFriendConfirm", "Remove {name} from your friends?", "{name} arkadaşlarından kaldırılsın mı?", "{name} aus deinen Freunden entfernen?", "Retirer {name} de vos amis ?", "¿Eliminar a {name} de tus amigos?", "Rimuovere {name} dai tuoi amici?", "{name} を友達から削除しますか？", "要将 {name} 从好友中移除吗？", "{name} 님을 친구에서 삭제할까요?", "{name} uit je vrienden verwijderen?", "Удалить {name} из друзей?")
add("noFriendsYet", "No friends yet. Share your code or add someone with theirs!", "Henüz arkadaşın yok. Kodunu paylaş veya birinin kodunu ekle!", "Noch keine Freunde. Teile deinen Code oder füge jemanden hinzu!", "Pas encore d'amis. Partagez votre code ou ajoutez quelqu'un !", "Aún no tienes amigos. ¡Comparte tu código o añade a alguien!", "Nessun amico ancora. Condividi il tuo codice o aggiungine uno!", "まだ友達がいません。コードを共有するか追加しましょう！", "还没有好友。分享你的代码或用对方的代码添加！", "아직 친구가 없습니다. 코드를 공유하거나 추가하세요!", "Nog geen vrienden. Deel je code of voeg iemand toe!", "Пока нет друзей. Поделитесь кодом или добавьте кого-нибудь!")
add("copyFriendCode", "Copy code", "Kodu kopyala", "Code kopieren", "Copier le code", "Copiar código", "Copia codice", "コードをコピー", "复制代码", "코드 복사", "Code kopiëren", "Скопировать код")
add("friendsSetupRequired", "Friends feature is not set up yet. Run supabase/add_friends.sql in the Supabase SQL Editor.", "Arkadaş özelliği henüz kurulmadı. Supabase SQL Editor'da supabase/add_friends.sql dosyasını çalıştırın.", "Freundesfunktion noch nicht eingerichtet. Führe supabase/add_friends.sql im Supabase SQL Editor aus.", "La fonction amis n'est pas encore configurée. Exécutez supabase/add_friends.sql dans l'éditeur SQL Supabase.", "La función de amigos aún no está configurada. Ejecuta supabase/add_friends.sql en el editor SQL de Supabase.", "La funzione amici non è ancora configurata. Esegui supabase/add_friends.sql nell'editor SQL di Supabase.", "友達機能がまだ設定されていません。Supabase SQL Editorで supabase/add_friends.sql を実行してください。", "好友功能尚未设置。请在 Supabase SQL 编辑器中运行 supabase/add_friends.sql。", "친구 기능이 아직 설정되지 않았습니다. Supabase SQL Editor에서 supabase/add_friends.sql을 실행하세요.", "Vriendenfunctie is nog niet ingesteld. Voer supabase/add_friends.sql uit in de Supabase SQL Editor.", "Функция друзей ещё не настроена. Выполните supabase/add_friends.sql в SQL Editor Supabase.")
add("notifications", "Notifications", "Bildirimler", "Benachrichtigungen", "Notifications", "Notificaciones", "Notifiche", "通知", "通知", "알림", "Meldingen", "Уведомления")
add("noNotifications", "No notifications", "Bildirim yok", "Keine Benachrichtigungen", "Aucune notification", "No hay notificaciones", "Nessuna notifica", "通知はありません", "暂无通知", "알림 없음", "Geen meldingen", "Нет уведомлений")
add("accept", "Accept", "Kabul Et", "Annehmen", "Accepter", "Aceptar", "Accetta", "承認", "接受", "수락", "Accepteren", "Принять")
add("reject", "Reject", "Reddet", "Ablehnen", "Refuser", "Rechazar", "Rifiuta", "拒否", "拒绝", "거절", "Weigeren", "Отклонить")
add("friendRequestSent", "Friend request sent to {name}", "{name} kullanıcısına arkadaşlık isteği gönderildi", "Freundschaftsanfrage an {name} gesendet", "Demande d'ami envoyée à {name}", "Solicitud de amistad enviada a {name}", "Richiesta di amicizia inviata a {name}", "{name} に友達申請を送信しました", "已向 {name} 发送好友请求", "{name} 님에게 친구 요청을 보냈습니다", "Vriendschapsverzoek verzonden naar {name}", "Запрос в друзья отправлен пользователю {name}")
add("friendRequestAlreadyExists", "A friend request already exists", "Zaten bekleyen bir arkadaşlık isteği var", "Eine Freundschaftsanfrage besteht bereits", "Une demande d'ami existe déjà", "Ya existe una solicitud de amistad", "Esiste già una richiesta di amicizia", "友達申請はすでに送信済みです", "好友请求已存在", "이미 친구 요청이 있습니다", "Er bestaat al een vriendschapsverzoek", "Запрос в друзья уже существует")
add("friendRequestReceived", "{name} sent you a friend request", "{name} sana arkadaşlık isteği gönderdi", "{name} hat dir eine Freundschaftsanfrage gesendet", "{name} vous a envoyé une demande d'ami", "{name} te envió una solicitud de amistad", "{name} ti ha inviato una richiesta di amicizia", "{name} から友達申請が届きました", "{name} 向你发送了好友请求", "{name} 님이 친구 요청을 보냈습니다", "{name} heeft je een vriendschapsverzoek gestuurd", "{name} отправил(а) вам запрос в друзья")
add("friendRequestApproved", "{name} accepted your friend request", "{name} arkadaşlık isteğini kabul etti", "{name} hat deine Freundschaftsanfrage angenommen", "{name} a accepté votre demande d'ami", "{name} aceptó tu solicitud de amistad", "{name} ha accettato la tua richiesta di amicizia", "{name} が友達申請を承認しました", "{name} 接受了你的好友请求", "{name} 님이 친구 요청을 수락했습니다", "{name} heeft je vriendschapsverzoek geaccepteerd", "{name} принял(а) ваш запрос в друзья")
add("friendRequestAccepted", "You and {name} are now friends", "Sen ve {name} artık arkadaşsınız", "Du und {name} seid jetzt Freunde", "Vous et {name} êtes maintenant amis", "Tú y {name} ahora sois amigos", "Tu e {name} ora siete amici", "{name} と友達になりました", "你和 {name} 现在是好友了", "{name} 님과 이제 친구입니다", "Jij en {name} zijn nu vrienden", "Вы и {name} теперь друзья")
